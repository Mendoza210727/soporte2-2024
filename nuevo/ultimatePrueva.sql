...
VALUES 
	...
	CREATE PROCEDURE PoblarReservasYPagos
...
BEGIN
    ...
    -- Cursor para recorrer todos los vuelos y generar reservas y pagos correspondientes
    ...
    WHILE @@FETCH_STATUS = 0
    BEGIN
        -- Genera @CantidadReservas para cada vuelo
        DECLARE @i INT = 1;
        ...
        BEGIN
            ...
            -- Genera una fecha de reserva entre 1 y 3 días antes de la fecha de vuelo
            SET @fecha_reserva = DATEADD(DAY, -(ABS(CHECKSUM(NEWID()) % 3 + 1)), @fecha_vuelo);
            ...
            -- Obtiene el número total de asientos disponibles para el vuelo
            SELECT @totalAsientos = COUNT(id_AsientoDisponible)
            FROM AsientoDisponible
            WHERE id_Vuelo = @id_Vuelo;

            -- Asegúrate de que haya asientos disponibles
            IF @totalAsientos > 0
            BEGIN
                -- Calcula la cantidad de asientos a reservar (1 a 4 asientos)
                SET @cantidadAsientosReserva = CAST((ABS(CHECKSUM(NEWID()) % 4) + 1) AS INT);

                -- Asegúrate de que no se intenten reservar más asientos de los disponibles
                IF @cantidadAsientosReserva > @totalAsientos
                BEGIN
                    SET @cantidadAsientosReserva = @totalAsientos;
                END

                -- Inserta los asientos en DetalleReserva (entre 1 y 4 disponibles)
                ...
                WHILE @@FETCH_STATUS = 0
                BEGIN
                    SET @descripcion = 'Asiento reservado';
                ...
                -- Genera detalles del pago con restricciones
                ...
                SET @monto = CAST(ABS(CHECKSUM(NEWID()) % 1000 + 1) AS DECIMAL(10, 2));
                SET @fecha_pago = DATEADD(DAY, ABS(CHECKSUM(NEWID()) % 2), @fecha_reserva);
                SET @estatus = CASE WHEN ABS(CHECKSUM(NEWID()) % 2) = 0 THEN 'Completado' ELSE 'Pendiente' END;
